...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const text = `Ad: ${formData.name}%0A Telefon: ${formData.phone}%0A Hizmet: ${formData.service}%0A Mesaj: ${formData.message}`;
    window.open(`https://wa.me/90${phoneNumber}?text=${text}`, "_blank");
  };

  const contactItems = [
    { icon: Phone, label: "Hizmet Bizi Ara", value: contact.phone, href: `tel:+90${contact.phone.replace(/\s/g, "")}` },
    { icon: Phone, label: "Acil Çağrı", value: contact.phone2 || contact.phone, href: `tel:+90${(contact.phone2 || contact.phone).replace(/\s/g, "")}` },
    { icon: Mail, label: "Email", value: contact.email, href: `mailto:${contact.email}` },
    { icon: MapPin, label: "Adres", value: contact.address, href: null },
    { icon: Clock, label: "Çalışma Saatleri", value: contact.hours, href: null },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <a
                href={`https://wa.me/90${contact.phone.replace(/\s/g, "")}`}
                target="_blank"
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label htmlFor="service" className="block text-sm font-medium text-blue-300 mb-2">
                  Hizmet
                </label>
                <select
                  id="service"
                ...
                >
                  ...
                  <option value="sehirlerarasi-cekici" className="bg-blue-900">Şehirlerarası Çekici</option>
                  ...
                  <option value="lastik-degisimi" className="bg-blue-900">Lastik Değişimi</option>
                  <option value="yakit-ikmali" className="bg-blue-900">Yakıt İkmali</option>
                  <option value="diger" className="bg-blue-900">Diğer</option>
                </select>
              </div>
